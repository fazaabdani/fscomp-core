...
export async function updateUnitAction(unitId: string, formData: FormData) {
  ...
  const updateData = {
    ...
    hasKeyboardBacklight: text(formData, "hasKeyboardBacklight") === "Ya",
    hasFingerprint: text(formData, "hasFingerprint") === "Ya",
    hasFaceRecognition: text(formData, "hasFaceRecognition") === "Ya",
    hasStylus: text(formData, "hasStylus") === "Ya",
    isFeatured: text(formData, "isFeatured") === "Ya",
