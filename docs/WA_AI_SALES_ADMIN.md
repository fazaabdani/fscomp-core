...
Intent `GENERAL_SERVICE` (servis umum, rakit PC — bukan klaim garansi) sekarang punya jalur sendiri di `decideWaAiPolicy()`: `AUTO_REPLY` tapi `notifyAdmin: true` dan `nextStatus: PENDING_ADMIN`, balasannya template tetap (`waAiGeneralServiceBridgeMessage()`, konfirmasi "Bisa" dulu baru arahkan cek kondisi/estimasi biaya ke admin) karena belum ada database riwayat servis untuk dibaca AI. Awalnya template ini langsung defer tanpa menjawab ("kami cekkan ulang dulu") — diubah 2026-08-04 karena customer bertanya ya/tidak sederhana ("kalo servis bisa?") butuh jawaban tegas dulu, bukan langsung dialihkan. Sebelumnya kata "servis"/"service" salah ke-match ke `WARRANTY` di `inferWaAiIntent()` — sudah diperbaiki, sekarang hanya "garansi"/"klaim" yang masuk `WARRANTY`.
...
- **AUTO**: `POST /api/wa/follow-up-sweep` (auth `CORE_INTEGRATION_TOKEN`, sama seperti endpoint n8n-facing lain) — dipanggil berkala dari luar. Sudah dipasang lewat **Coolify Scheduled Task** (`wa-ai-follow-up-sweep`, jalan tiap jam `0 * * * *`, command `node -e "fetch(...)"` langsung dari container app-nya sendiri — tidak lewat n8n, konsisten dengan prinsip Tahap 8). Endpoint ini no-op kalau mode bukan `AUTO`.
...
## Tahap 11: Bug — Intent Tidak Dikenal Bikin AI Macet Permanen

Ditemukan dari tes langsung: customer menjawab "Saya Mba Mba bukan Mas" (koreksi sapaan) — kalimat ini tidak cocok keyword mana pun di `inferWaAiIntent()`, jatuh ke `UNKNOWN`. Sebelum perbaikan ini, `decideWaAiPolicy()` memperlakukan `UNKNOWN` sebagai "tidak aman", langsung `HANDOVER_ADMIN` dengan `nextStatus: PENDING_ADMIN` — dan begitu status jadi `PENDING_ADMIN`, cabang paling atas `decideWaAiPolicy()` (`status === WAITING_ADMIN || PENDING_ADMIN`) langsung mengunci **semua pesan berikutnya** ke `HANDOVER_ADMIN` juga, apa pun isinya. Akibatnya di tes: pertanyaan lanjutan yang jelas-jelas aman ("Stok yang ready apa saja Kak?", "Budget saya sekitar 3 jutaan") ikut kena template generik "saya teruskan ke admin" — padahal seharusnya STOCK_SIMPLE/CATALOG dan dijawab AI. Sekali macet, tidak ada jalan keluar otomatis — cuma bisa dibetulkan manual lewat tombol "Kembalikan ke AI".

Perbaikan di `lib/wa-ai-policy.ts`:

- `UNKNOWN` dipindah ke `safeIntents` (bukan lagi jatuh ke catch-all `unknown_or_unsafe`). Alasannya: semua intent yang benar-benar berisiko (komplain, refund, garansi, bayar, nego, minta admin) sudah ditangkap cabang `adminRequiredIntents`/`RISK` lebih dulu — begitu sampai ke titik ini, "tidak cocok keyword" jauh lebih sering berarti obrolan biasa yang daftar kata kuncinya belum lengkap, bukan sesuatu yang genuinely berbahaya.
...
## Tahap 13: Intent `TROUBLESHOOTING` (Gejala Teknis Umum) + Sapaan Konsisten per Nomor

Dua permintaan Faza dari sesi tes langsung yang sama (2026-08-04):

1. Pertanyaan umum soal gejala/masalah teknis (misal "laptop saya kadang mati sendiri", "kenapa lemot") mau dijawab AI dulu (analisa awal) sebelum diarahkan bawa unit ke toko untuk cek pasti — bukan langsung full handover kosong seperti komplain sungguhan.
2. Sapaan ke customer harus konsisten: kalau nomornya sudah pernah ngobrol dan sapaannya sudah diketahui/dikoreksi, pakai itu terus. Kalau nomor baru, default netral "Kak" — jangan menebak "Mas"/"Mbak" (ini akar masalah bug "Saya Mba Mba bukan Mas" di Tahap 11).

**Intent `TROUBLESHOOTING`** (`lib/wa-ai-policy.ts`):

- `inferWaAiIntent()`: kata kunci gejala umum (`rusak|bermasalah|tidak normal|mati|error|lemot|lag|nge-hang|ngadat`) dipindah dari `COMPLAINT` ke intent baru `TROUBLESHOOTING`. `COMPLAINT` sekarang cuma kata eksplisit (`komplain|keluhan`) — tetap full `HANDOVER_ADMIN` seperti sebelumnya, tidak berubah.
- `decideWaAiPolicy()`: `TROUBLESHOOTING` dengan `customerPolicy AUTO_SAFE` → `AUTO_REPLY`, `allowSafeCatalog: true` (beda dari `GENERAL_SERVICE` yang balasannya template tetap — di sini AI beneran generate jawaban lewat `generateWaAiCatalogReply()`), `notifyAdmin: true`, `nextStatus: "OPEN"` (belajar dari bug Tahap 12, sengaja tidak dibikin `PENDING_ADMIN`).
- Aturan wajib baru di `hardSafetyPreamble` (`lib/wa-ai-catalog-reply.ts`, aturan #6): AI boleh kasih insight kemungkinan penyebab, tapi wajib tutup dengan arahkan bawa unit ke toko (Kajen/Wiradesa) untuk cek pasti — tidak boleh mendiagnosis pasti atau menjanjikan hasil servis/garansi tanpa cek fisik.

**Sapaan konsisten** (field baru `WaCustomer.preferredGreeting`, migration `202608040003_wa_customer_preferred_greeting`):

- Default `null` untuk customer baru → system prompt AI diinstruksikan pakai "Kak" (netral, tidak menebak gender).
- `detectWaGreetingCorrection()` di `lib/wa-ai-policy.ts`: regex sempit yang cuma nyala kalau ada pola eksplisit "bukan mas"/"bukan mbak" (persis kasus nyata "Saya Mba Mba bukan Mas") — sengaja tidak bereaksi ke kata "mas"/"mbak" yang muncul biasa di kalimat (mis. ngomongin orang lain) supaya tidak salah simpan.
- `processWaIncomingMessage()` (`lib/wa-ai-orchestrator.ts`) menjalankan deteksi ini di setiap pesan masuk, update `WaCustomer.preferredGreeting` kalau ketemu koreksi baru, lalu meneruskan nilainya ke `generateWaAiCatalogReply()` supaya konsisten dipakai di semua balasan berikutnya dari nomor yang sama — termasuk balasan yang barusan dikoreksi.
- **Catatan operasional**: migration ini (dan migration-migration sebelumnya) tidak otomatis jalan saat deploy — `Dockerfile`/`package.json` di repo ini cuma jalankan `prisma generate` + `next build`, tidak ada `prisma migrate deploy` di build/start script. Ternyata memang sudah rutin dijalankan manual oleh Faza tiap deploy (dikonfirmasi 2026-08-04: `npx prisma migrate deploy` di container menunjukkan "27 migrations found ... No pending migrations to apply" sebelum PR ini, jadi bukan proses yang selama ini terlewat) — lanjutkan kebiasaan yang sama tiap migration baru ditambahkan.

## Tahap 14: Persona Diganti Total oleh Faza + Aturan Wajib Baru "Lokasi Unit"

2026-08-05, Faza mengganti total `ai_sales_persona_prompt` lewat dashboard `/wa-ai` jadi framework closing sales yang jauh lebih detail (tahapan baca posisi customer, teknik closing, cara menangani keberatan, dll) — ini murni perubahan data di database, tidak menyentuh kode sama sekali, jadi tidak ada PR terkait isi personanya. `defaultWaAiPersonaPrompt` di kode (`lib/wa-ai-catalog-reply.ts`) cuma fallback kalau setting DB kosong — sudah diselaraskan juga dengan instruksi gaya (emoji pembuka + list lengkap kalau diminta) tapi ini tidak berpengaruh ke behavior live milik Faza karena setting DB-nya sudah ada isi.

Satu permintaan dari sesi yang sama masuk kode (bukan cuma persona), karena ini soal keselamatan transaksi bukan gaya bicara: **aturan wajib #7** di `hardSafetyPreamble` — setiap kali AI membahas unit tertentu (ditanyakan atau direkomendasikan), wajib sebutkan lokasinya sekarang (Kajen/Wiradesa, dari field `lokasi` di STOK_TERSEDIA yang sudah dikirim ke model). Alasan: kalau tidak disebutkan tiap kali, ada risiko nyata customer datang ke toko yang ternyata tidak ada unitnya. Sengaja dijadikan hardcoded (bukan ditambahkan ke persona yang bisa diedit/hilang admin) karena kelasnya sama dengan aturan wajib lain (jangan mengarang stok/harga, dst) — bukan preferensi gaya yang boleh berubah-ubah.
