import { Boxes, ClipboardList, Cpu } from "lucide-react";
import { formatRupiah } from "@/lib/api";
import { pcCategoryLabels } from "@/lib/pc-builder";
import { prisma } from "@/lib/prisma";
import { requireRole } from "@/lib/session";
import { createPcComponentAction, createPcPresetAction, togglePcComponentAction, updatePcDraftStatusAction } from "./actions";
...
export default async function PcBuilderAdminPage({ searchParams }: { searchParams?: Record<string,string|undefined> }) {
  ...
  return <section className="pageStack">
    ...
    {searchParams?.error ? <div className="infoBox dangerInfo">Data belum lengkap atau nilai unik sudah digunakan.</div>:null}
    ...
    <form className="panel formGrid" action={createPcComponentAction}><div className="panelHeader"><div><p className="eyebrow">Katalog komponen</p><h2>Tambah komponen</h2></div></div><div className="formGrid twoColumns">
      <label>Kategori<select name="category">{categories.map(item=><option value={item} key={item}>{pcCategoryLabels[item]}</option>)}</select></label><label>Nama<input name="name" required placeholder="AMD Ryzen 5 5600"/></label><label>Merek<input name="brand" placeholder="AMD"/></label><label>Harga jual<input name="salePrice" type="number" min="0" step="1000" required/></label>
      <label>Item Inventaris<select name="inventoryItemId"><option value="">Tidak ditautkan</option>{inventory.map(item=><option value={item.id} key={item.id}>{item.name} {item.brandModel} {item.serialNumber ? `/ SN ${item.serialNumber}`:""}</option>)}</select></label><label>Socket<input name="socket" placeholder="AM4 / LGA1700"/></label><label>Tipe memori<input name="memoryType" placeholder="DDR4 / DDR5"/></label><label>Form factor<input name="formFactor" placeholder="ATX / mATX / ITX"/></label><label>Watt/TDP<input name="wattage" type="number" min="0" placeholder="650"/></label><label className="wideField">Spesifikasi<textarea name="specification" placeholder="Detail singkat yang tampil di builder"/></label></div><button className="primaryButton" type="submit">Tambah Komponen</button></form>
    <section className="panel"><div className="panelHeader"><div><p className="eyebrow">Data aktif</p><h2>Daftar komponen</h2></div></div><div className="tableLike compact">{components.map(item=><div className="unitRow" key={item.id}><span className="unitNumber">{pcCategoryLabels[item.category]}</span><span><strong>{item.name}</strong><small>{item.specification||"-"} {item.inventoryItem ? `/ Inventaris ${item.inventoryItem.status}`:"/ katalog manual"}</small></span><b>{formatRupiah(item.salePrice)}</b><form action={togglePcComponentAction.bind(null,item.id)}><button className="secondaryButton compactButton" type="submit">{item.active?"Nonaktifkan":"Aktifkan"}</button></form></div>)}</div></section>
    <form className="panel formGrid" action={createPcPresetAction}><div className="panelHeader"><div><p className="eyebrow">Paket pilihan</p><h2>Buat preset</h2></div></div><div className="formGrid twoColumns"><label>Nama<input name="name" required placeholder="Gaming Hemat"/></label><label>Slug<input name="slug" required placeholder="gaming-hemat"/></label><label>Kebutuhan<input name="useCase" required placeholder="Gaming 1080p"/></label><label>Deskripsi<input name="description" placeholder="Cocok untuk..."/></label></div><div className="checkboxGrid">{components.filter(i=>i.active).map(item=><label key={item.id}><input type="checkbox" name="componentId" value={item.id}/>{pcCategoryLabels[item.category]} - {item.name}</label>)}</div><button className="primaryButton" type="submit">Simpan Preset</button></form>
