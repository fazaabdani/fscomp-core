...
export default async function PcBuilderAdminPage({ searchParams }: { searchParams?: Record<string,string|undefined> }) {
  ...
  return <section className="pageStack">
    ...
    <section className="panel"><div className="panelHeader"><div><p className="eyebrow">Data aktif</p><h2>Daftar komponen</h2></div></div><div className="tableLike compact">{components.map(item=><div className="unitRow" key={item.id}><span className="unitNumber">{pcCategoryLabels[item.category]}</span><span><strong>{item.name}</strong><small>{item.specification||"-"} {item.inventoryItem ? `/ Inventaris ${item.inventoryItem.status}`:"/ katalog manual"}</small></span><b>{formatRupiah(item.salePrice)}</b><div className="buttonRow noMargin"><Link className="secondaryButton compactButton" href={`/rakit-pc/components/${item.id}/edit`}><Edit3 size={14}/> Edit</Link><form action={togglePcComponentAction.bind(null,item.id)}><button className="secondaryButton compactButton" type="submit">{item.active?"Nonaktifkan":"Aktifkan"}</button></form></div></div>)}</div></section>
