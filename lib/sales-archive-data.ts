  ...
  return {
    rows: sales.map((sale) => {
      ...
      return {
        ...
        model: sale.unit?.model ?? "Lisensi / software tanpa laptop",
        specification: sale.unit ? `${sale.unit.processor} / ${sale.unit.ram} / ${sale.unit.ssd}` : "Transaksi non-laptop",
        buyerName: sale.buyerName ?? "-",
        location: sale.location === "WIRADESA" ? "Wiradesa" : "Kajen",
        paymentMethod: sale.paymentMethod,
        laptopCost: laptopItem ? laptopItem.unitCost * laptopItem.qty : 0,
        laptopCostSource: laptopItem ? "Snapshot item laptop" : "Tidak ada laptop",
